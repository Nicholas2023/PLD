...
int builtin_help(_st *nick)
{
	...
	mensajes[1] = EXIT_HELP_INFO;
	mensajes[2] = ENVIRONMENT_HELP_INFO;
	mensajes[3] = SETENV_HELP_INFO;
	mensajes[4] = UNSETENV_HELP_INFO;
	mensajes[5] = CD_HELP_INFO;

	for (i = 0; mensajes[i]; i++)
	{
		length = str_length(nick->f[1]);
		if (str_compare(nick->f[1], mensajes[i], length))
		{
